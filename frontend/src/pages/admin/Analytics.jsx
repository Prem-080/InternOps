...
import { BarChart3, Trophy, TrendingUp, Building2, Calendar, Filter } from 'lucide-react';
    ...
    <div className="animate-fade-in-up">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Performance & attendance insights</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              {topPerformers.map((u, idx) => (
                <div key={u.id} className="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
                  <span className="flex items-center gap-3 font-medium text-gray-700">
                    <span className="text-lg w-6 text-center">{MEDAL[idx] || `#${idx + 1}`}</span>
                    {u.full_name || u.email}
                  </span>
        ...
        <Card className="p-6">
          <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
            <TrendingUp className="w-5 h-5 text-green-500" /> Attendance Trends (6 mo)
          </h3>
          {!byMonth.length ? (
            <p className="text-gray-400 text-sm">No data yet.</p>
          ) : (
      ...
      <Card className="p-6">
        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
          <Building2 className="w-5 h-5 text-indigo-500" /> Department Attendance
        </h3>
        
        <div className="flex flex-wrap gap-4 mb-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
          <div className="flex-1 min-w-[200px]">
             <label className="text-xs font-semibold text-gray-500 uppercase mb-1 block flex items-center gap-1"><Filter className="w-3 h-3"/> Department</label>
             <select
              value={deptId}
        ...
        {!deptId ? (
          <p className="text-gray-400 text-sm italic">Select a department to view detailed attendance metrics.</p>
        ) : !deptAttendance ? (
          <Spinner />
        ) : (
