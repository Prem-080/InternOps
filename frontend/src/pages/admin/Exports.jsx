...
import { Download, CalendarDays, Star, Target, ArrowDownToLine, AlertCircle } from 'lucide-react';
...
export default function Exports() {
  ...
  const download = async (endpoint, requiresDates) => {
    ...
    try {
      ...
      a.href = url;
      a.download = requiresDates ? `${endpoint}-${from}-${to}.csv` : `${endpoint}.csv`;
      a.click();
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800 tracking-tight">Export Reports</h1>
          <p className="text-sm text-gray-500 mt-0.5">Download CSV data for any date range</p>
        </div>
      </div>
        ...
        {EXPORTS.map((e) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${e.grad} text-white flex items-center justify-center shadow-md mb-4`}>
                  {e.icon}
                </div>
                
                <h3 className="font-bold text-gray-900 text-lg mb-1">{e.label} CSV</h3>
